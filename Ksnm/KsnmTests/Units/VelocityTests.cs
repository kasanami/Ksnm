    ...
    public class VelocityTests
    {
        ...
        public void VelocityTest()
        {
            ...
            // キロメートル毎時
            {
                ...
                var velocity2 = (MetrePerSecond<Ksnm.Numerics.Decimal>)velocity;
            ...
            // ノット
            {
                ...
                // 秒速に変換
                velocity2 = (MetrePerSecond<Ksnm.Numerics.Decimal>)velocity;
                Assert.AreEqual("0.5144444444444444444444444444m/s", velocity2.ToString());

                // 時速に変換
                var velocity3 = (KiloMetrePerHour<Ksnm.Numerics.Decimal>)velocity;
                Assert.AreEqual("1.852km/h", velocity3.ToString());

                // 再度ノットに変換
                velocity = (Knot<Ksnm.Numerics.Decimal>)velocity3;
                Assert.AreEqual("1kt", velocity.ToString());

                // 再度ノットに変換 精度の限界で正確に1ktにはならない
                velocity = (Knot<Ksnm.Numerics.Decimal>)velocity2;
